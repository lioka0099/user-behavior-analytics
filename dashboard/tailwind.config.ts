...
// Tailwind v4 still supports config files, but Tremor relies on:
// - custom theme tokens like `fill-tremor-content`
// - dynamically generated color utility classes like `fill-orange-500`
// Without these, Tremor charts often fall back to black fills.
const config: Config = {
  // darkMode: ["class"],
  content: [
  ...
  // Tremor generates lots of color utility class names dynamically at runtime
  // (e.g. `fill-${color}-${shade}`), so Tailwind needs a safelist.
  safelist: [
    {
      pattern:
